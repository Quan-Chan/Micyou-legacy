...
var TonalPalette = class _TonalPalette {
  ...
  static fromInt(argb) {
    ...
    return _TonalPalette.fromHct(hct2);
  }
  /**
   * @param hct Hct
   * @return Tones matching that color's hue and chroma.
   */
  static fromHct(hct2) {
    return new _TonalPalette(hct2.hue, hct2.chroma, hct2);
  }
  /**
   * @param hue HCT hue
...
var TemperatureCache = class _TemperatureCache {
  ...
  analogous(count = 5, divisions = 12) {
    ...
    for (let i = 0; i < 360; i++) {
      ...
      const hct2 = this.hctsByHue[hue];
      const temp = this.relativeTemperature(hct2);
   ...
   */
  relativeTemperature(hct2) {
    const range = this.tempsByHct.get(this.warmest) - this.tempsByHct.get(this.coldest);
    const differenceFromColdest = this.tempsByHct.get(hct2) - this.tempsByHct.get(this.coldest);
    if (range === 0) {
      return 0.5;
    }
...
var Score = class _Score {
  ...
  static score(colorsToPopulation, options) {
    ...
    for (const hct2 of colorsHct) {
      const hue = sanitizeDegreesInt(Math.round(hct2.hue));
      const proportion = hueExcitedProportions[hue];
      if (filter && (hct2.chroma < _Score.CUTOFF_CHROMA || proportion <= _Score.CUTOFF_EXCITED_PROPORTION)) {
        continue;
      }
      const proportionScore = proportion * 100 * _Score.WEIGHT_PROPORTION;
      const chromaWeight = hct2.chroma < _Score.TARGET_CHROMA ? _Score.WEIGHT_CHROMA_BELOW : _Score.WEIGHT_CHROMA_ABOVE;
      const chromaScore = (hct2.chroma - _Score.TARGET_CHROMA) * chromaWeight;
      const score = proportionScore + chromaScore;
      scoredHct.push({ hct: hct2, score });
    }
    scoredHct.sort(compare);
    const chosenColors = [];
    for (let differenceDegrees2 = 90; differenceDegrees2 >= 15; differenceDegrees2--) {
      chosenColors.length = 0;
      for (const { hct: hct2 } of scoredHct) {
        const duplicateHue = chosenColors.find((chosenHct) => {
          return differenceDegrees(hct2.hue, chosenHct.hue) < differenceDegrees2;
        });
        if (!duplicateHue) {
          chosenColors.push(hct2);
        }
    ...
    if (chosenColors.length === 0) {
      colors2.push(fallbackColorARGB);
    }
    for (const chosenHct of chosenColors) {
      colors2.push(chosenHct.toInt());
    }
    return colors2;
  }
};
...
var hct = Hct.fromInt(argbFromHex("#4A672D"));
var schemeLight = new SchemeTonalSpot(hct, false, 0);
var schemeDark = new SchemeTonalSpot(hct, true, 0);
var colors = [
  "primary",
  "onPrimary",
  "primaryContainer",
